...
public class HeliusCipher {
	...
	public HeliusCipher(long key) {
		privateCipherKey = key * CIPHER_NUMBER;
	}
	...
	public static String decode(String encoded, int key) {
		...
		for(int s = 0; s < value.length(); s += SUB_SIZE) {
			String sub = value.substring(s, s + SUB_SIZE);
			
			long par = Long.parseLong(sub) - (key * CIPHER_NUMBER);
	...
	private static char getByShort(short code) {
		for(Map.Entry<Character, Short> e : ALPHABETICAL_NUMBERS.entrySet()) {
	...
	public String encode(String normal) {
		...
		for(char c : normal.toCharArray()) {
			short b = ALPHABETICAL_NUMBERS.getOrDefault(c, (short) 184);
			String value = "" + ((b * b) + privateCipherKey);
			
			if(value.length() < SUB_SIZE)
	...
	private String upToEight(String full) {
		...
		for(int i = 0; i < SUB_SIZE && ne.length() < SUB_SIZE; i++) {
			ne.insert(0, "0");
		}
		return ne.toString();
	}
	...
	public String decode(String encoded) {
		...
		for(int s = 0; s < value.length(); s += SUB_SIZE) {
			...
			long par = Long.parseLong(sub) - privateCipherKey;
